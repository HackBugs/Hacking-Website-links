' HackBugs PC Advanced Toolkit v3 - Colorful & Visible Menu + Browser Chooser (37)
' Updated: Chrome uses exact path C:\Program Files\Google\Chrome\Application\chrome.exe
' Mumbai Edition - Feb 2026

Option Explicit
Dim WShell, FSO, choice, subchoice, cmd, title, browserChoice
...
Do
    ...
    Select Case Trim(choice)
        ...
        Case "37"   ' Choose browser to open
            ...
            Else
                Select Case Trim(browserChoice)
                    ...
                    Case "2"   ' Chrome - using your exact path
                        Dim chromePath
                        ...
                        Else
                            MsgBox "Chrome not found at:" & vbCrLf & chromePath & vbCrLf & vbCrLf & _
                                   "Please check if Chrome is installed there.", vbExclamation, title
                            cmd = ""
                        End If

                    Case "3"   ' Firefox
                        cmd = WShell.ExpandEnvironmentStrings("%ProgramFiles%\Mozilla Firefox\firefox.exe")
                        If Not FSO.FileExists(cmd) Then cmd = "firefox.exe"

                    Case "4"   ' Opera
                        cmd = WShell.ExpandEnvironmentStrings("%ProgramFiles%\Opera\opera.exe")
                        If Not FSO.FileExists(cmd) Then cmd = "opera.exe"

                    Case "5"   ' Brave
                        cmd = WShell.ExpandEnvironmentStrings("%ProgramFiles%\BraveSoftware\Brave-Browser\Application\brave.exe")
                        If Not FSO.FileExists(cmd) Then cmd = "brave.exe"

                    Case Else
                        MsgBox "Invalid choice (use 1-5 only).", vbExclamation, title
                        cmd = ""
                End Select
            End If
    ...
    If cmd <> "" Then
        ...
        If Err.Number <> 0 Then
            MsgBox "Launch failed!" & vbCrLf & vbCrLf & _
                   ...
                   "Tip: Run as admin if needed.", vbCritical, title
        Else
